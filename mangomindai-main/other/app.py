...
When ever the user asks questions such as" tell me about the document", summerise key points in the document.
Only say "I don't understand the question" if the context is completely unrelated.
...
def router_node(state: GraphState):
   ...
   elif any(w in {"prime minister", "president", "prime"} for w in words):
       return {"route": "google"}
...
def fallback_node(state: GraphState):
   reply = "I don't know based on the provided document. Can you try rephrasing your question?"
...
def google_node(state:GraphState):
    conversation = [
        {"role": "system",
        "content": "You are a helpful search assistant for general questions, you must answer only to the user questions"
        },
    ]

    message = state["question"].lower()
